...
export function buildApp() {
  const config=loadConfig(), app=Fastify({logger:{level:config.logLevel}});
  const tools=new ToolRegistry(), gateway=new StubModelGateway();
  ...
  app.get("/api/v1/status",async()=>({status:"ok",identity:MOROK_IDENTITY,capabilities:{commands:coreCommands.length,permissions:corePermissions.length,tools:tools.list().length,modelGateway:true}}));
  ...
  app.post("/api/v1/messages",async(request,reply)=>{const body=request.body as {message?:string};if(!body?.message?.trim())return reply.code(400).send({error:"message_required"});return gateway.complete({message:body.message.trim()})});
  app.get("/health/database",async(_request,reply)=>{const client=new MongoClient(config.mongodbUri,{serverSelectionTimeoutMS:3000});try{await client.connect();const db=client.db(config.mongodbDatabase);await db.command({ping:1});await initializeDatabase(db);return{status:"ok",service:"mongodb",database:config.mongodbDatabase}}catch{return reply.code(503).send({status:"error",service:"mongodb"})}finally{await client.close().catch(()=>undefined)}});
