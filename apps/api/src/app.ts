...
export function buildApp(){
  ...
  app.post("/api/v1/messages",async(req,reply)=>{
    ...
    const body=req.body as {message?:string;conversationId?:string}; if(!body?.message?.trim())return reply.code(400).send({error:"message_required"});
